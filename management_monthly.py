...
def display_vendor_analysis(raw_df, month, biz_name):
    ...
    try:
        df = raw_df.iloc[:, [0, 1, 2, 16]]
        df.columns = ['Month', 'Amount', 'Biz', 'Vendor']
        ...
        with c1:
            ...
            fig.update_layout(height=400, barmode='group', plot_bgcolor='white', xaxis_tickangle=-45)
            st.plotly_chart(fig, use_container_width=True)
        with c2:
    ...
    except: st.info(f"{biz_name} 전표 데이터 부족")
...
if selected_mode == "연결 실적(통합)":
    ...
    if k in ["온리프", "르샤인", "오블리브"]:
        st.divider()
        h_s = [get_val(raw_dfs[conf["sheet"]], conf["병원매출"], col_maps[k][m]) for m in sel_months]
        h_p = [get_val(raw_dfs[conf["sheet"]], conf["병원영익"], col_maps[k][m]) for m in sel_months]
        ...
        p_s = [get_val(raw_dfs[conf["sheet"]], conf["법인매출"], col_maps[k][m]) for m in sel_months]
        p_p = [get_val(raw_dfs[conf["sheet"]], conf["법인영익"], col_maps[k][m]) for m in sel_months]

...
def display_vendor_analysis(raw_df, month, biz_name):
    ...
    try:
        ...
        # D열(Category)에서 "03.매출원가-의약품비"만 정확히 필터링
        ...
        st.info("의약품비 상세 전표 데이터를 분석할 수 없습니다.")
...
try:
    ...
    if selected_mode == "연결 실적(통합)":
        ...
        ts = [get_val(raw_dfs["온리프_실적"], CONFIG["온리프"]["전체매출"], col_maps["온리프"][m]) + get_val(raw_dfs["르샤인_실적"], CONFIG["르샤인"]["전체매출"], col_maps["르샤인"][m]) + get_val(raw_dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["전체매출"], col_maps["오블리브"][m]) for m in sel_months]
        tp = [get_val(raw_dfs["온리프_실적"], CONFIG["온리프"]["전체영익"], col_maps["온리프"][m]) + get_val(raw_dfs["르샤인_실적"], CONFIG["르샤인"]["전체영익"], col_maps["르샤인"][m]) + get_val(raw_dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["전체영익"], col_maps["오블리브"][m]) + get_val(raw_dfs["HQ_실적"], CONFIG["메디빌더"]["영익"], col_maps["메디빌더"][m]) for m in sel_months]
        ...
        st.divider()
        cs = [get_val(raw_dfs["HQ_실적"], CONFIG["메디빌더"]["매출"], col_maps["메디빌더"][m]) + get_val(raw_dfs["온리프_실적"], CONFIG["온리프"]["법인매출"], col_maps["온리프"][m]) + get_val(raw_dfs["르샤인_실적"], CONFIG["르샤인"]["법인매출"], col_maps["르샤인"][m]) + get_val(raw_dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["법인매출"], col_maps["오블리브"][m]) for m in sel_months]
        cp = [get_val(raw_dfs["온리프_실적"], CONFIG["온리프"]["법인영익"], col_maps["온리프"][m]) + get_val(raw_dfs["르샤인_실적"], CONFIG["르샤인"]["법인영익"], col_maps["르샤인"][m]) + get_val(raw_dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["법인영익"], col_maps["오블리브"][m]) + get_val(raw_dfs["HQ_실적"], CONFIG["메디빌더"]["영익"], col_maps["메디빌더"][m]) for m in sel_months]
        ...
        if k in ["온리프", "르샤인", "오블리브"]:
            st.divider()
            h_total_s = [get_val(raw_dfs[conf["sheet"]], conf["병원매출"], col_maps[k][m]) for m in sel_months]
            h_profit = [get_val(raw_dfs[conf["sheet"]], conf["병원영익"], col_maps[k][m]) for m in sel_months]
            if conf.get("hosp_items"):
                h_sales_dict = {"Total": h_total_s}
                for item_name, item_row in conf["hosp_items"].items(): h_sales_dict[item_name] = [get_val(raw_dfs[conf["sheet"]], item_row, col_maps[k][m]) for m in sel_months]
            ...
            p_sales = [get_val(raw_dfs[conf["sheet"]], conf["법인매출"], col_maps[k][m]) for m in sel_months]
            p_profit = [get_val(dfs[conf["sheet"]].iloc[conf["법인영익"]-1, col_maps[k][m]] / 1000000 if col_maps[k][m] is not None else 0) for m in sel_months] # 기존 로직 복구
            draw_performance_chart(f"🤝 {k} 앤파트너스 실적", sel_months, {"Total": p_sales, "앤파트너스 매출": p_sales}, [get_val(raw_dfs[conf["sheet"]], conf["법인영익"], col_maps[k][m]) for m in sel_months], conf["color"])
            st.divider(); st.subheader(f"📑 {k} 5대 핵심 비용 분석")
            c1, c2 = st.columns(2)
            with c1:
                draw_expense_chart("① 인건비(병원) 분석", sel_months, h_total_s, [get_val(raw_dfs[conf["sheet"]], conf["인건비_병원"], col_maps[k][m]) for m in sel_months], "인건비(병)", conf["color"], "#A8DADC")
                draw_expense_chart("③ 의약품비 분석", sel_months, h_total_s, [get_val(raw_dfs[conf["sheet"]], conf["의약품비"], col_maps[k][m]) for m in sel_months], "의약품비", conf["color"], "#457B9D")
                draw_expense_chart("⑤ 광고선전비 분석", sel_months, h_total_s, [get_val(raw_dfs[conf["sheet"]], conf["광고비"], col_maps[k][m]) for m in sel_months], "광고비", conf["color"], "#F1FAEE")
            with c2:
                draw_expense_chart("② 인건비(앤파) 분석", sel_months, p_sales, [get_val(raw_dfs[conf["sheet"]], conf["인건비_앤파"], col_maps[k][m]) for m in sel_months], "인건비(앤파)", conf["color"], "#A8DADC")
                draw_expense_chart("④ 상품매입 분석", sel_months, h_total_s, [get_val(raw_dfs[conf["sheet"]], conf["상품매입"], col_maps[k][m]) for m in sel_months], "상품매입", conf["color"], "#F4A261")
            
            # [추가] BU별 페이지 하단에 거래처 분석 배치

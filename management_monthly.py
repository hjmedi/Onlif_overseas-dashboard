...
CONFIG = {
    ...
    "온리프": {
        ...
        "color": "#1f77b4", "centers": {} # 온리프는 단일 센터
    },
    "르샤인": {
        ...
        "color": "#006400",
        "centers": {"강남": 79, "홍대": 80, "신사": 81, "인천": 82, "수원": 83, "부산": 84}
    },
    "오블리브": {
        ...
        "color": "#8B4513",
        "centers": {"송도": 79, "부평": 80, "배곧": 81, "부천": 82}
    }
}
...
# [중요] 매출(막대)+영업이익(꺾은선) 통합 차트 함수 (센터별 누적 지원)
def draw_performance_chart(title, months, sales_data, total_profit, main_color):
    """
    sales_data: { "CenterName": [values...], "Total": [values...] } 형태의 딕셔너리
    """
    st.markdown(f"### {title}")
    fig = go.Figure()

    # 1. 매출 막대 (센터가 여러 개면 누적 막대, 아니면 단일 막대)
    total_sales = sales_data.get("Total", [])
    if len(sales_data) > 1: # 센터별 데이터가 있는 경우
        for center_name, values in sales_data.items():
            if center_name == "Total": continue
            fig.add_trace(go.Bar(x=months, y=values, name=center_name))
        fig.update_layout(barmode='stack')
    else: # 단일 매출 데이터인 경우
        fig.add_trace(go.Bar(x=months, y=total_sales, name="매출", marker_color="#E0E0E0", opacity=0.8))

    # 2. 영업이익 꺾은선 (매출 위에 겹치기)
    fig.add_trace(go.Scatter(
        x=months, y=total_profit, name="영업이익", mode="lines+markers+text", 
        line=dict(color=main_color, width=4), 
        text=[f"{v/100:.1f}억" for v in total_profit], textposition="top center"
    ))

    # 3. 전체 매출 텍스트 표기 (막대 맨 위에 총합 표시)
    fig.add_trace(go.Scatter(
        x=months, y=total_sales, mode="text", name="총매출",
        text=[f"{v/100:.1f}억" for v in total_sales], textposition="top center",
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="금액 (백만 원)"), xaxis=dict(type='category'), hovermode="x unified"
    )
    st.plotly_chart(fig, use_container_width=True)

# (나머지 display_metrics, draw_expense_chart 함수 등은 이전과 동일...)
...
try:
    ...
    if selected_mode == "연결 실적(통합)":
        ...
        draw_performance_chart("📊 그룹 전체 연결 실적 (매출 막대 / 영업이익 선)", sel_months, {"Total": ts}, tp, "#E91E63")
        st.divider()
        ...
        if k in ["온리프", "르샤인", "오블리브"]:
            st.divider()
            # [특수] 병원(의원) 상세 차트 - 센터별 누적 적용
            h_sales_data = {"Total": [get_val(dfs[k], conf["병원매출"], maps[k][m]) for m in sel_months]}
            for c_name, c_row in conf["centers"].items():
                h_sales_data[c_name] = [get_val(dfs[k], c_row, maps[k][m]) for m in sel_months]
            
            h_profit = [get_val(dfs[k], conf["병원영익"], maps[k][m]) for m in sel_months]
            draw_performance_chart(f"🏥 {k} 의원 센터별 매출 및 이익 추이", sel_months, h_sales_data, h_profit, conf["color"])
            ...
            draw_performance_chart(f"🤝 {k} 앤파트너스 실적", sel_months, {"Total": p_sales}, p_profit, "#9E9E9E")

            # 비용 분석 차트 (기존 유지)
            st.divider(); st.subheader(f"📑 {k} 5대 핵심 비용 분석")
            h_sales_list = h_sales_data["Total"]
            p_sales_list = p_sales
            c1, c2 = st.columns(2)
            with c1:
                draw_expense_chart("① 인건비(병원) 분석", sel_months, h_sales_list, [get_val(dfs[k], conf["인건비_병원"], maps[k][m]) for m in sel_months], "인건비(병)", "#4B8BBE")
                draw_expense_chart("③ 의약품비 분석", sel_months, h_sales_list, [get_val(dfs[k], conf["의약품비"], maps[k][m]) for m in sel_months], "의약품비", "#306998")
                draw_expense_chart("⑤ 광고선전비 분석", sel_months, h_sales_list, [get_val(dfs[k], conf["광고비"], maps[k][m]) for m in sel_months], "광고비", "#FFD43B")
            with c2:
                draw_expense_chart("② 인건비(앤파) 분석", sel_months, p_sales_list, [get_val(dfs[k], conf["인건비_앤파"], maps[k][m]) for m in sel_months], "인건비(앤파)", "#6495ED")
                draw_expense_chart("④ 상품매입 분석", sel_months, h_sales_list, [get_val(dfs[k], conf["상품매입"], maps[k][m]) for m in sel_months], "상품매입", "#FFE873")

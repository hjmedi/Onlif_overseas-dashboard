...
def display_vendor_analysis(raw_df, month, biz_name):
    ...
    except: st.info("의약품비 상세 전표 데이터를 분석할 수 없습니다.")
...
try:
    ...
    if selected_mode == "연결 실적(통합)":
        ...
        ts = [get_val(dfs["온리프_실적"], CONFIG["온리프"]["전체매출"], col_maps["온리프"][m]) + get_val(dfs["르샤인_실적"], CONFIG["르샤인"]["전체매출"], col_maps["르샤인"][m]) + get_val(dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["전체매출"], col_maps["오블리브"][m]) for m in sel_months]
        tp = [get_val(dfs["온리프_실적"], CONFIG["온리프"]["전체영익"], col_maps["온리프"][m]) + get_val(dfs["르샤인_실적"], CONFIG["르샤인"]["전체영익"], col_maps["르샤인"][m]) + get_val(dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["전체영익"], col_maps["오블리브"][m]) + get_val(dfs["HQ_실적"], CONFIG["메디빌더"]["영익"], col_maps["메디빌더"][m]) for m in sel_months]
        ...
        st.divider()
        cs = [get_val(dfs["HQ_실적"], CONFIG["메디빌더"]["매출"], col_maps["메디빌더"][m]) + get_val(dfs["온리프_실적"], CONFIG["온리프"]["법인매출"], col_maps["온리프"][m]) + get_val(dfs["르샤인_실적"], CONFIG["르샤인"]["법인매출"], col_maps["르샤인"][m]) + get_val(dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["법인매출"], col_maps["오블리브"][m]) for m in sel_months]
        cp = [get_val(dfs["온리프_실적"], CONFIG["온리프"]["법인영익"], col_maps["온리프"][m]) + get_val(dfs["르샤인_실적"], CONFIG["르샤인"]["법인영익"], col_maps["르샤인"][m]) + get_val(dfs["오블리브(송도)_실적"], CONFIG["오블리브"]["법인영익"], col_maps["오블리브"][m]) + get_val(dfs["HQ_실적"], CONFIG["메디빌더"]["영익"], col_maps["메디빌더"][m]) for m in sel_months]
        ...
        if k in ["온리프", "르샤인", "오블리브"]:
            ...
            if conf.get("hosp_items"):
                ...
                for item_name, item_row in conf["hosp_items"].items(): h_sales_dict[item_name] = [get_val(dfs[conf["sheet"]], item_row, col_maps[k][m]) for m in sel_months]

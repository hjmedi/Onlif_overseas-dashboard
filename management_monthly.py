...
def draw_expense_chart(title, months, sales_list, exp_list, exp_label, line_color, bar_color):
    ...
    fig.add_trace(go.Bar(
        ...
        opacity=0.8, # 막대를 더 진하게
        marker_line_color=bar_color, marker_line_width=1.5, # 막대 테두리 추가
    ...
    fig.update_layout(
        title=dict(text=f"<b>{title}</b>", font=dict(size=18)), height=380, margin=dict(l=10,r=10,t=60,b=10), 
        yaxis=dict(title="금액 (백만 원)", showgrid=False), 
        yaxis2=dict(title="비중 (%)", overlaying="y", side="right", range=[0, max(ratios)*1.6 if ratios else 30], showgrid=False), 
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), 
        xaxis=dict(type='category'), plot_bgcolor="white", hovermode="x unified"
    )
    st.plotly_chart(fig, use_container_width=True)
...
try:
    ...
    if selected_mode == "연결 실적(통합)":
        ...
        if k in ["온리프", "르샤인", "오블리브"]:
            ...
            with c2:
                draw_expense_chart("② 인건비(앤파) 분석", sel_months, [get_val(dfs[k], conf["법인매출"], maps[k][m]) for m in sel_months], [get_val(dfs[k], conf["인건비_앤파"], maps[k][m]) for m in sel_months], "인건비(앤파)", conf["color"], "#A8DADC")

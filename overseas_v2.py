...
# 1. 페이지 기본 설정
st.set_page_config(page_title="온리프 해외 매출 검증", layout="wide")
...
if raw_data is not None:
    ...
    # 1️⃣ [국내/해외 필터] T열 근처의 구분 컬럼에서 '해외'만 추출
    ...
    # 2️⃣ [매출액 계산] 알려주신 '수납액 (CRM)' 컬럼 사용
    # 제목이 조금 다를 경우를 대비해 '수납액' 글자가 포함된 열을 찾습니다.
    amt_col = [c for c in df.columns if '수납액' in c and 'CRM' in c]
    if not amt_col: # 만약 못 찾으면 '수납액' 글자로 재검색
    ...
    st.title("💰 온리프 해외 매출 실시간 검증 (CRM 데이터)")
    
    # 상단 요약 지표
    total_rev = df['매출액_숫자'].sum()
    st.metric(label="총 해외 매출 합계 (수납액 기준)", value=f"{total_rev:,.0f} 원")
    st.info(f"📍 현재 분석된 해외 환자: {len(df)}명")
    ...
    if not df.empty:
        ...
        # 표에 보여줄 컬럼 선택
        show_cols = [c for c in ['고객등록일', '이름', '국적', amt_col[0] if amt_col else None] if c in df.columns]
        st.dataframe(df[show_cols], use_container_width=True)
    else:
        st.warning("분석할 '해외' 환자 데이터가 없습니다. 시트의 구분(T열)을 확인해 주세요.")
else:
    st.error("데이터 로딩 실패")

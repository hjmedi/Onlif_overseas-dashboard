...
COMMISSION_RATES = {"레이블": 0.15, "The SC": 0.15, "천수현 대표": 0.15, "앤티스": 0.15, "크리에이트립": 0.11}

# (기존 함수 동일: get_region, to_numeric, get_dynamic_ticks)
...
if not month_list:
    ...
    start_month, end_month = st.sidebar.select_slider("조회 기간 선택", options=CHRONOLOGICAL_MONTHS, value=(default_start, default_end))
    ...
    # ==========================================================
    if menu == "🌐 온리프 해외매출 전체":
        st.title(f"🌐 {sel_month} 온리프 해외매출 분석 {title_suffix}")
        ...
        if '매출월' in df_main.columns:
            ...
            if is_latest_month and latest_date:
                ...
                total_days_in_month = calendar.monthrange(latest_date.year, latest_date.month)[1]
            ...
            if idx < len(month_list) - 1:
                ...
                # 🚀 성장률 비교 시 마감된 달은 실제값으로, 진행 중인 달은 '예상 마감치'로 비교
            ...
            with c2:
                if is_latest_month:
                    st.metric("🎯 예상 마감 매출", f"{projected_rev:,.0f}원", f"{growth_rate:+.1f}% (전월比)", help="현재까지의 일평균 매출로 월말까지의 실적을 예측한 수치입니다.")
                else:
            ...
            # 2. 예상 마감액 추가 (최신 월인 경우에만 겹쳐서 표시)
            if is_latest_month and sel_month in FILTERED_MONTHS:
                # 투명도가 있는 회색 박스로 예상치 표시
                fig.add_trace(go.Bar(
                    x=[sel_month], y=[projected_rev], 
                    name="예상 마감액", 
    ...
    # ==========================================================
    # --- 메뉴 2: 수수료 매출 (동일 로직 적용 가능하나 생략) ---
    # ==========================================================
    else:
        st.info("수수료 매출 메뉴에서도 위와 동일한 방식으로 예상 마감치를 확인할 수 있습니다.")
